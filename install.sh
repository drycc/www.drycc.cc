...
function install_mountpoint {
  ...
  version=$(curl -Ls $mountpoint_api_url/releases|grep /drycc/mountpoint-s3-csi-driver/releases/tag/ | sed -E 's/.*\/drycc\/mountpoint-s3-csi-driver\/releases\/tag\/([0-9\.]{1,}(-rc.[0-9]{1,})?)".*/\1/g' | head -1)
